...
class MADDPG:
    def __init__(
        ...
        self.actors = [Actor(dim_obs, dim_act, n_units) for i in range(n_agents)]
        self.critics = [
            Critic(n_agents, dim_obs, dim_act, n_units) for i in range(n_agents)
        ]
    ...
    def select_action(self, state_batch):
        ...
        for i in range(self.n_agents):
            ...
            act += th.from_numpy(np.random.randn(actions.shape[-1]) * self.var[i]).type(
                FloatTensor
            )
